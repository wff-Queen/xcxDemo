...
Page({
    ...
    onLoad: function() {
        ...
        self.mapCtx = wx.createMapContext('myMap')
            // 2WGBZ-VH733-ZMG3Z-3KRXJ-FMTC3-KCBXZ实例化API核心类
                    ...
                    if (self.data.currentRegion.city == name) {
                        qqmapsdk.getDistrictByCityId({
                            ...
                            success: function(res) { //成功后的回调
                                ...
                                console.log(self.data.regionData)
                            },
                            fail: function(error) {
                                //console.error(error);
                            },
    ...
    selectedOk: function() {
        ...
        wx.showModal({
            title: '提示',
            content: this.data.centerData.title + '--' + this.data.centerData.addr,
            success: function(res) {
                if (res.confirm) {
                    console.log('用户点击确定')
                } else if (res.cancel) {
